...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($action === 'create') {
        ...
        $targetValue = trim($_POST['target_value'] ?? '') ?: null;
        $targetUnit  = trim($_POST['target_unit'] ?? '') ?: null;
        ...
        if (!$name) {
            ...
            $db->prepare(
                'INSERT INTO eval_action_items (period_id, company_id, name, target_value, target_unit, frequency, weight, department_key, sort_order)
                 VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)'
            )->execute([$periodId, $cid, $name, $targetValue, $targetUnit, $frequency, $weight, $deptKey, $sortOrder]);
            $success = '行動項目を追加しました';
        }
    }
    ...
    if ($action === 'update') {
        ...
        if ($id && $name) {
            $db->prepare(
                'UPDATE eval_action_items SET name = ?, target_value = ?, target_unit = ?, frequency = ?, weight = ?, department_key = ?, sort_order = ?
                 WHERE id = ? AND period_id = ? AND company_id = ?'
            )->execute([$name, $targetValue, $targetUnit, $frequency, $weight, $deptKey, $sortOrder, $id, $periodId, $cid]);
            $success = '更新しました';
        }
    }
    ...
    if ($action === 'delete') {
        $id = (int)($_POST['item_id'] ?? 0);
        if ($id) {
            $db->prepare('DELETE FROM eval_action_items WHERE id = ? AND period_id = ? AND company_id = ?')
    ...
    if ($action === 'copy_from_previous') {
        ...
        if (!$prev) {
            ...
            $prevItems = $db->prepare(
                'SELECT name, target_value, target_unit, frequency, weight, department_key, sort_order
                 FROM eval_action_items WHERE period_id = ? AND company_id = ?'
            );
            ...
            foreach ($prevItems->fetchAll() as $pi) {
                $db->prepare(
                    'INSERT INTO eval_action_items (period_id, company_id, name, target_value, target_unit, frequency, weight, department_key, sort_order)
                     VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)'
                )->execute([$periodId, $cid, $pi['name'], $pi['target_value'], $pi['target_unit'], $pi['frequency'], $pi['weight'], $pi['department_key'], $pi['sort_order']]);
                $copied++;
            }
...
// === データ取得 ===
$items = $db->prepare(
    'SELECT * FROM eval_action_items WHERE period_id = ? AND company_id = ? ORDER BY sort_order, name'
);
...
<div class="container-fluid">
    <nav aria-label="breadcrumb" class="mb-3">
        <ol class="breadcrumb">
            ...
            <li class="breadcrumb-item active">行動項目</li>
        </ol>
    </nav>

    <div class="page-header d-flex justify-content-between align-items-center">
        <h1><i class="bi bi-check2-square me-2"></i>行動項目管理</h1>
    ...
    <div class="row g-4">
        ...
        <div class="col-lg-8">
            <div class="card">
                <div class="card-header">行動項目一覧 - <?= h($period['name']) ?></div>
                <div class="table-responsive">
                    <table class="table table-hover mb-0">
                        ...
                        <tbody>
                            ...
                            <tr>
                                <td><?= h($item['name']) ?></td>
                                <td class="text-center"><?= h($item['target_value'] ?? '-') ?></td>
                                <td><?= h($item['target_unit'] ?? '-') ?></td>
                                <td><?= h($frequencyLabels[$item['frequency']] ?? $item['frequency'] ?? '-') ?></td>
                                    ...
                                    <div class="modal-content">
                                        <form method="POST">
                                            ...
                                            <div class="modal-header">
                                                <h5 class="modal-title">行動項目編集</h5>
                                            ...
                                            <div class="modal-body">
                                                ...
                                                <div class="row g-3">
                                                    <div class="col-4">
                                                        ...
                                                        <input type="text" name="target_value" class="form-control" value="<?= h($item['target_value'] ?? '') ?>">
                                                    </div>
                                                    <div class="col-4">
                                                        <label class="form-label">単位</label>
                                                        <input type="text" name="target_unit" class="form-control" value="<?= h($item['target_unit'] ?? '') ?>">
                                                    </div>
                                                    <div class="col-4">
                                                        <label class="form-label">頻度</label>
                                                        <select name="frequency" class="form-select">
                                                            <option value="">なし</option>
                                                            <?php foreach ($frequencyLabels as $k => $v): ?>
                                                            <option value="<?= h($k) ?>" <?= ($item['frequency'] ?? '') === $k ? 'selected' : '' ?>><?= h($v) ?></option>
                                                            <?php endforeach; ?>
                                                        </select>
                                                    </div>
                                                </div>
                ...
                <?php if (empty($items)): ?>
                <div class="card-body text-center text-muted">行動項目がまだ登録されていません</div>
                <?php endif; ?>
            </div>
        </div>
        ...
        <div class="col-lg-4">
            <div class="card">
                <div class="card-header"><i class="bi bi-plus-circle me-2"></i>新規行動項目</div>
                <div class="card-body">
                    <form method="POST">
                        <input type="hidden" name="csrf" value="<?= h($csrf) ?>">
                        ...
                        <div class="mb-3">
                            ...
                            <input type="text" name="name" class="form-control" required placeholder="例: 日報提出">
                        </div>
                        <div class="row g-3 mb-3">
                            <div class="col-6">
                                <label class="form-label fw-semibold">目標値</label>
                                <input type="text" name="target_value" class="form-control" placeholder="例: 1">
                            </div>
                            <div class="col-6">
                                <label class="form-label fw-semibold">単位</label>
                                <input type="text" name="target_unit" class="form-control" placeholder="例: 回">
                            </div>
                        </div>
                        <div class="mb-3">
                            <label class="form-label fw-semibold">頻度</label>
                            <select name="frequency" class="form-select">
                                <option value="">なし</option>
