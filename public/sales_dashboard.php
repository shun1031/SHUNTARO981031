    ...
    <div class="row g-4 mb-4">
        ...
        <div class="col-lg-6">
            <div class="card h-100">
                <div class="card-header"><i class="bi bi-people me-1" style="color:#3b82f6"></i>スタッフ人数分析</div>
                <div class="card-body">
                    <div class="row g-0">
                        <div class="col-5">
                            <div class="sales-chart-wrap" style="height:200px">
                                <canvas id="staffPieChart"></canvas>
                            </div>
                        </div>
                        <div class="col-7 ps-3">
                            <table class="table table-sm mb-0" style="font-size:.82rem">
                                <thead><tr><th style="white-space:nowrap">項目</th><th class="text-end">今月</th><th>前年同月比</th></tr></thead>
                                <tbody>
                                    <?php if ($caseTypeFilter === ''): ?>
                                    <tr>
                                        <td style="white-space:nowrap">常勤比率</td>
                                        <td class="text-end"><?= $empStats['regular_rate'] ?>%<br><small class="text-muted"><?= $empStats['regular'] ?>名</small></td>
                                        <td><?= $fmtYoy($empStats['regular_rate'], $empStatsYoy['regular_rate'], '%') ?></td>
                                    </tr>
                                    <tr>
                                        <td style="white-space:nowrap">イベント比率</td>
                                        <td class="text-end"><?= $empStats['event_rate'] ?>%<br><small class="text-muted"><?= $empStats['event'] ?>名</small></td>
                                        <td><?= $fmtYoy($empStats['event_rate'], $empStatsYoy['event_rate'], '%') ?></td>
                                    </tr>
                                    <?php else: ?>
                                    <tr>
                                        <td class="fw-bold" style="white-space:nowrap">総人数</td>
                                        <td class="text-end fw-bold"><?= $wTotal ?>件</td>
                                        <td class="text-muted small">-</td>
                                    </tr>
                                    <tr>
                                        <td style="white-space:nowrap">自社比率</td>
                                        <td class="text-end"><?= $wInhouseRate ?>%<br><small class="text-muted"><?= $workerGrouped['自社']['case_count'] ?>件</small></td>
                                        <td class="text-muted small">-</td>
                                    </tr>
                                    <tr>
                                        <td style="white-space:nowrap">外注比率</td>
                                        <td class="text-end"><?= $wAlliRate ?>%<br><small class="text-muted"><?= $workerGrouped['アライアンス']['case_count'] ?>件</small></td>
                                        <td class="text-muted small">-</td>
                                    </tr>
                                    <tr>
                                        <td style="white-space:nowrap">自社粗利率</td>
                                        <td class="text-end"><?= $wInhouseMargin ?>%</td>
                                        <td class="text-muted small">-</td>
                                    </tr>
                                    <tr>
                                        <td style="white-space:nowrap">外注粗利率</td>
        ...
        <div class="col-lg-6">
            <div class="card h-100">
                <div class="card-header d-flex justify-content-between align-items-center">
                    <span><i class="bi bi-pie-chart me-1" style="color:#8b5cf6"></i>スタッフ区分別売上</span>
                    <div class="btn-group btn-group-sm" role="group">
                ...
                <div class="card-body">
                    ...
                    <div class="mt-3">
                        <?php foreach ($workerGrouped as $groupName => $gd): ?>
                        <div class="d-flex justify-content-between align-items-center mb-2 p-2 rounded" style="background:<?= $groupName === '自社' ? '#eff6ff' : '#f0fdf4' ?>">
                            <span class="fw-bold" style="color:<?= $groupName === '自社' ? '#3b82f6' : '#059669' ?>"><?= h($groupName) ?></span>
                            <div class="text-end">
                                <div class="fw-bold worker-revenue" data-raw="<?= $gd['revenue'] ?>"><?= number_format($gd['revenue']) ?>円</div>
                            </div>
                        </div>
                        <?php endforeach; ?>
                    </div>
