...
if (!isset($_GET['month']) && $_SERVER['REQUEST_METHOD'] !== 'POST') {
    ...
    redirect(BASE_PATH . '/public/sales_regular.php?year=' . $defaultYear . '&month=' . $defaultMonth);
}
...
if (isset($_GET['export'])) {
    ...
    $filters = ['case_type' => 'regular', 'year' => $_GET['year'] ?? '', 'month' => $_GET['month'] ?? '', 'client_id' => $_GET['client_id'] ?? '', 'status' => $_GET['status'] ?? '', 'employee_name' => $csvEmpFilter ?? ''];
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && verifyCsrfToken($_POST['csrf'] ?? '')) {
    ...
    if ($action === 'create' || $action === 'update') {
        ...
        $data = [
            ...
            'days_worked'    => (int)($_POST['months_count'] ?? 1),
    ...
    $_backBase  = BASE_PATH . '/public/sales_regular.php?year=' . $_backYear . '&month=' . $_backMonth;
    if ($action === 'cancel') { cancelSalesCase((int)$_POST['id'], $cid); redirect($_backBase . '&msg=' . urlencode('キャンセルしました')); }
    if ($action === 'delete') { deleteSalesCase((int)$_POST['id'], $cid); redirect($_backBase . '&msg=' . urlencode('削除しました')); }
    if ($action === 'copy_prev_month') {
        ...
        $prevStmt = $db->prepare("SELECT * FROM sales_cases WHERE company_id=? AND case_year=? AND case_month=? AND case_type='regular' AND status!='cancelled'");
        ...
        foreach ($prevStmt->fetchAll() as $pc) {
            createSalesCase($cid, [
                ...
                'end_date' => '', 'sales_rep' => $pc['sales_rep'], 'manager' => $pc['manager'],
        ...
        redirect(BASE_PATH . '/public/sales_regular.php?year=' . $curYear . '&month=' . $curMonth . '&msg=' . urlencode($copied . '件コピーしました'));
    }
}
...
$_acSql = "SELECT DISTINCT client_id FROM sales_cases WHERE company_id=? AND case_type='regular' AND case_year=? AND status NOT IN ('cancelled','終了') AND client_id IS NOT NULL";
$_acParams = [$cid, $year];
...
// 屋号・店舗名の選択肢（過去のDB値）
$_tns = $db->prepare("SELECT DISTINCT trade_name FROM sales_cases WHERE company_id=? AND case_type='regular' AND trade_name IS NOT NULL AND trade_name != '' ORDER BY trade_name");
$_tns->execute([$cid]);
$distinctTradeNames = $_tns->fetchAll(PDO::FETCH_COLUMN);
$_sns = $db->prepare("SELECT DISTINCT store_name FROM sales_cases WHERE company_id=? AND case_type='regular' AND store_name IS NOT NULL AND store_name != '' ORDER BY store_name");
$_sns->execute([$cid]);
$distinctStoreNames = $_sns->fetchAll(PDO::FETCH_COLUMN);

// KPI集計（フィルター全条件を反映）
$kpiWhere = "company_id = ? AND case_year = ? AND case_type = 'regular' AND status != 'cancelled'";
$kpiParams = [$cid, $year];
    ...
    <div class="page-header">
        <div class="d-flex justify-content-between align-items-center flex-wrap gap-2">
            ...
            <div class="d-flex gap-2 align-items-center">
                ...
                <button type="button" class="btn btn-outline-primary btn-sm" onclick="openImportModal()"><i class="bi bi-clipboard-data me-1"></i>一括インポート</button>
    ...
    <div class="modal-dialog modal-sm">
        <div class="modal-content">
            ...
            <div class="modal-body">
                ...
                <small class="text-muted">前月の常勤案件データを今月にコピーします。</small>
            </div>
...
<div class="modal fade" id="caseModal" tabindex="-1">
    <div class="modal-dialog modal-lg">
        <form method="post" class="modal-content case-form" id="caseFormEl">
            ...
            <input type="hidden" name="case_type" value="regular">
            <div class="modal-header">
                <h5 class="modal-title" id="modalTitle">常勤案件追加</h5>
                <button type="button" class="btn-close" data-bs-dismiss="modal"></button>
            </div>
            <div class="modal-body">
                <div class="row g-3">
                    ...
                    <div class="col-md-3">
                        <label class="form-label fw-medium">支払単価(月)</label>
...
<div class="modal fade" id="importModal" tabindex="-1">
    <div class="modal-dialog modal-xl modal-dialog-scrollable">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title"><i class="bi bi-clipboard-data me-2"></i>常勤案件 一括インポート</h5>
                <button type="button" class="btn-close" data-bs-dismiss="modal"></button>
            </div>
            <div class="modal-body">
                <!-- STEP1: 貼り付け -->
                <div id="impStep1">
                    <div class="d-flex align-items-center gap-2 mb-3 flex-wrap">
                        <span class="fw-medium">登録先の年月</span>
                        <select id="impYear" class="form-select form-select-sm" style="width:110px">
                            <?php for ($y = (int)date('Y') + 1; $y >= 2024; $y--): ?>
                            <option value="<?= $y ?>" <?= $y === $dispYear ? 'selected' : '' ?>><?= $y ?>年</option>
                            <?php endfor; ?>
                        </select>
                        <select id="impMonth" class="form-select form-select-sm" style="width:90px">
                            <?php for ($m = 1; $m <= 12; $m++): ?>
                            <option value="<?= $m ?>" <?= $m === $dispMonth ? 'selected' : '' ?>><?= $m ?>月</option>
                            <?php endfor; ?>
                        </select>
                        <span class="text-muted small">この年月の「開始月」で登録され、同じ月の一覧に表示されます</span>
                    </div>

                    <div class="alert alert-secondary py-2 small mb-3">
                        <div class="fw-semibold mb-1"><i class="bi bi-info-circle me-1"></i>スプレッドシートの行をそのままコピーして貼り付けてください（タブ区切り）</div>
                        <div style="font-size:.78rem;line-height:1.9">
                            列の順番：<br>
                            <span class="badge bg-primary">1 取引先</span>
                            <span class="badge bg-primary">2 営業担当</span>
                            <span class="badge bg-primary">3 管理者</span>
                            <span class="badge bg-primary">4 採用者</span>
                            <span class="badge bg-primary">5 スタッフ区分</span>
                            <span class="badge bg-primary">6 外注先</span>
                            <span class="badge bg-primary">7 スタッフ名</span>
                            <span class="badge bg-primary">8 稼働開始日</span>
                            <span class="badge bg-primary">9 稼働日数</span>
                            <span class="badge bg-primary">10 キャリア</span>
                            <span class="badge bg-primary">11 店舗名</span>
                            <span class="badge bg-secondary">12〜15 使いません</span>
                            <span class="badge bg-primary">16 売上(月額)</span>
                            <span class="badge bg-primary">17 原価(月額)</span>
                            <span class="badge bg-secondary">18以降 使いません</span>
                        </div>
                        <div class="mt-2" style="font-size:.78rem">
                            ・<strong>売上・原価は16・17列目</strong>の月額を使います（14・15列目の単価は使いません）<br>
                            ・スタッフ区分の「外注」は<strong>個人外注</strong>として登録します<br>
                            ・<strong>屋号・区分（1次/2次以降）・終了月</strong>は元データに列がないため空欄で登録します<br>
                            ・稼働開始日は<strong>備考</strong>に「稼働開始 2025/12」として残します<br>
                            ・ステータスは<strong>確定</strong>で登録します
                        </div>
                    </div>

                    <textarea id="impData" class="form-control" rows="10" style="font-family:ui-monospace,SFMono-Regular,Menlo,monospace;font-size:.78rem"
                              placeholder="ここにスプレッドシートの行を貼り付け"></textarea>
                    <div id="impError" class="alert alert-danger py-2 small mt-2 mb-0" style="display:none"></div>
                </div>
                        ...
                        <table class="table table-sm table-hover mb-0" style="font-size:.74rem">
                            <thead class="table-light" style="position:sticky;top:0;z-index:1">
                                <tr>
                                    ...
                                    <th class="text-end">請求単価(月)</th>
                                    <th class="text-end">支払単価(月)</th>
                                    <th class="text-end">粗利</th>
                                    <th>備考／注意</th>
                                </tr>
                            </thead>
            ...
            <div class="modal-footer">
                <button type="button" class="btn btn-secondary" data-bs-dismiss="modal" id="impCloseBtn">キャンセル</button>
                <button type="button" class="btn btn-outline-secondary" id="impBackBtn" style="display:none" onclick="impBackToStep1()">貼り付けに戻る</button>
                <button type="button" class="btn btn-primary" id="impPreviewBtn" onclick="impPreview()"><i class="bi bi-eye me-1"></i>内容を確認</button>
                <button type="button" class="btn btn-success" id="impCommitBtn" style="display:none" onclick="impCommit()"><i class="bi bi-check-circle me-1"></i>この内容で登録</button>
            </div>
        </div>
    </div>
</div>
...
$inlineJs .= 'var importApiUrl = ' . json_encode(BASE_PATH . '/public/api/import_regular_cases.php') . ';';
$inlineJs .= <<<'IMPJS'
...
function impYen(n) { return (parseInt(n) || 0).toLocaleString(); }
...
function impBackToStep1() {
    ...
    document.getElementById('impPreviewBtn').style.display = '';
    document.getElementById('impCommitBtn').style.display = 'none';
    document.getElementById('impBackBtn').style.display = 'none';
}

function impPreview() {
    var data = document.getElementById('impData').value;
    var err  = document.getElementById('impError');
    if (!data.trim()) {
        err.textContent = 'データを貼り付けてください。';
        err.style.display = '';
        return;
    }
    var btn = document.getElementById('impPreviewBtn');
    btn.disabled = true; btn.innerHTML = '確認中...';
    ...
    fetch(importApiUrl, { method: 'POST', body: fd })
        .then(function(r) { return r.json(); })
        .then(function(d) {
            btn.disabled = false; btn.innerHTML = '<i class="bi bi-eye me-1"></i>内容を確認';
            if (!d.success) {
                err.textContent = d.error || '確認に失敗しました。';
                err.style.display = '';
                return;
            }
            if (!d.count) {
                err.textContent = '登録できる行が見つかりませんでした。列がタブ区切りになっているか確認してください。';
                err.style.display = '';
                return;
            }
            impRenderPreview(d);
...
function impRenderPreview(d) {
    _impRowCount = d.count;
    ...
    if (d.new_clients.length) {
        s += '<div class="alert alert-warning py-2 small mb-2"><i class="bi bi-plus-circle me-1"></i><strong>新しく作成される取引先（' + d.new_clients.length + '件）:</strong> '
           + d.new_clients.map(impEsc).join('、')
           + '<div class="mt-1" style="font-size:.75rem">既に似た名前が登録されていないか確認してください（表記違いで重複登録される場合があります）。</div></div>';
    }
    if (d.new_alliances.length) {
        s += '<div class="alert alert-warning py-2 small mb-2"><i class="bi bi-plus-circle me-1"></i><strong>新しく作成される外注先（' + d.new_alliances.length + '件）:</strong> '
           + d.new_alliances.map(impEsc).join('、') + '</div>';
    }
    if (d.skipped.length) {
        s += '<div class="alert alert-secondary py-2 small mb-2"><i class="bi bi-slash-circle me-1"></i><strong>取り込まない行（' + d.skipped.length + '行）:</strong><ul class="mb-0 mt-1">';
        d.skipped.forEach(function(k) {
            s += '<li>' + k.line + '行目 — ' + impEsc(k.reason) + '（' + impEsc(k.text) + '）</li>';
        });
        s += '</ul></div>';
    }
    document.getElementById('impSummary').innerHTML = s;

    // 明細
    var html = '';
    d.rows.forEach(function(r, i) {
        var warnHtml = '';
        if (r.note) warnHtml += '<div class="text-muted">' + impEsc(r.note) + '</div>';
        ...
        html += '<td>' + impEsc(r.manager) + '</td>';
        html += '<td>' + impEsc(r.recruiter) + '</td>';
        html += '<td>' + impEsc(r.worker_type) + '</td>';
        html += '<td>' + impEsc(r.alliance_name) + '</td>';
        html += '<td class="fw-medium">' + impEsc(r.worker_name) + '</td>';
        html += '<td>' + impEsc(r.carrier) + '</td>';
        html += '<td>' + impEsc(r.store_name) + '</td>';
        html += '<td class="text-end">' + r.days_worked + '</td>';
        html += '<td class="text-end">' + impYen(r.unit_price_in) + '</td>';
        html += '<td class="text-end">' + impYen(r.unit_price_out) + '</td>';
        html += '<td class="text-end ' + (r.gross_profit < 0 ? 'text-danger fw-bold' : '') + '">' + impYen(r.gross_profit) + '</td>';
        html += '<td style="font-size:.68rem">' + warnHtml + '</td>';
        html += '</tr>';
    });
    document.getElementById('impPreviewBody').innerHTML = html;
...
function impCommit() {
    ...
    fetch(importApiUrl, { method: 'POST', body: fd })
        .then(function(r) { return r.json(); })
        .then(function(d) {
            btn.disabled = false; btn.innerHTML = '<i class="bi bi-check-circle me-1"></i>この内容で登録';
            if (!d.success) {
                alert('登録に失敗しました: ' + (d.error || '不明なエラー'));
                return;
            }
            var h = '<div class="alert alert-success"><div class="fw-semibold"><i class="bi bi-check-circle me-1"></i>'
                  + impEsc(d.target) + ' に ' + d.created + '件を登録しました</div>';
            if (d.created_clients)   h += '<div class="small mt-1">取引先を' + d.created_clients + '件 新規作成しました</div>';
            if (d.created_alliances) h += '<div class="small">外注先を' + d.created_alliances + '件 新規作成しました</div>';
            h += '</div>';
            if (d.failed && d.failed.length) {
                h += '<div class="alert alert-danger small"><strong>登録できなかった行（' + d.failed.length + '件）</strong><ul class="mb-0 mt-1">';
                d.failed.forEach(function(f) {
                    h += '<li>' + f.line + '行目 ' + impEsc(f.worker_name) + ' — ' + impEsc(f.error) + '</li>';
                });
                h += '</ul></div>';
            }
            h += '<p class="text-muted small mb-0">一覧に反映されました。このウィンドウは閉じて構いません。</p>';

            document.getElementById('impStep1').style.display = 'none';
            document.getElementById('impStep2').style.display = 'none';
            document.getElementById('impStep3').innerHTML = h;
            document.getElementById('impStep3').style.display = '';
            document.getElementById('impCommitBtn').style.display = 'none';
            document.getElementById('impBackBtn').style.display = 'none';
            document.getElementById('impCloseBtn').textContent = '閉じる';
